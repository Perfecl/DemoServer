...
void PK::InitPlayer(const sy::OtherPlayerInfo* other_player, bool attacker,
  ...
  for (int i = 0; i < other_player->battle_pos1_size(); ++i) {
    ...
    if (!found) heros.push_back(sy::HeroInfo());
  }

  sy::CurrentCarrierInfo carrier = other_player->carrier();
  this->InitMonsterInfo(attacker, carrier, heros, hp);
}

void PK::InitMonsterInfo(bool attacker, sy::CurrentCarrierInfo& carrier,
                         std::vector<sy::HeroInfo>& monster,
                         const int64_t* hp) {
  PkObject& o = attacker ? this->a : this->b;
  o.Clear();
  INFO_LOG(logger)("Init Player, HP:%ld,%ld,%ld,%ld,%ld,%ld",
  ...
  for (size_t i = 0; i < heros.size(); ++i) {
    ...
    ++this->hero_uid;
    if (heros[i].hero_id()) {
      heros[i].set_uid(attacker ? this->hero_uid : -this->hero_uid);
      int64_t current_hp = hp && hp[pos - 1] > 0 ? hp[pos - 1] : 0;
      if (current_hp >= 0) {
        o.AddHero(*this, heros[i], pos, attacker ? false : true, false,
                  current_hp);
